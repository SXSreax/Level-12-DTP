...
def compare_result(id1, id2):
    with get_db() as db:
        ...
        cursor.execute("""SELECT name, image_url,
                       description FROM Hero WHERE id = ?""", (id1,))
        h1 = cursor.fetchone()
        cursor.execute("""SELECT ability_name,
                       description FROM Abilities WHERE hero_id = ?""", (id1,))
        h1_abilities = cursor.fetchall()
        cursor.execute("""SELECT skin_name,
                       skin_image_url FROM Skins WHERE hero_id = ?""", (id1,))
        h1_skins = cursor.fetchall()

        # Fetch hero 2 details
        cursor.execute("""SELECT name, image_url,
                       description FROM Hero WHERE id = ?""", (id2,))
        h2 = cursor.fetchone()
        cursor.execute("""SELECT ability_name,
                       description FROM Abilities WHERE hero_id = ?""", (id2,))
        h2_abilities = cursor.fetchall()
        cursor.execute("""SELECT skin_name,
                       skin_image_url FROM Skins WHERE hero_id = ?""", (id2,))
        h2_skins = cursor.fetchall()

    # If either hero is missing, show error page
    if h1 is None or h2 is None:
        return render_template('404.html',
                               message="One or both heroes not found!"), 404
